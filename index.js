...
async function fetchNSE(path, retry=true) {
  ...
  const result = await fetchURL('www.nseindia.com', path, {
    'User-Agent':      'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/122.0.0.0 Safari/537.36',
  ...
  if (!result.ok && result.error && result.error.includes('HTML') && retry) {
    console.log('NSE HTML response — refreshing cookie and retrying');
    NSE_COOKIE = '';
    await refreshCookie();
...
function fetchYahoo(symbols) {
  ...
  const path = '/v8/finance/spark?symbols='+syms+'&range=1d&interval=1d';
  return new Promise(resolve => {
    const req = https.get({
      ...
      stream.on('end', () => {
        ...
        catch(e) { resolve({ ok:false, error:e.message+' raw:'+data.slice(0,100) }); }
      });
...
async function runFullRefresh(label) {
  ...
  // Wait 2 seconds after cookie before hitting NSE API
  // NSE needs time to register the session
  ...
  // Use getQuotes bulk endpoint — much faster than individual calls
  // NSE getQuotes accepts comma-separated symbols
  for (const batch of priceBatches) {
    const symsStr = batch.map(s => encodeURIComponent(s)).join('%2C');
    const r = await fetchNSE('/api/getQuotes?symbol='+symsStr+'&series=%5B%22EQ%22%5D');
    if (r.ok && r.data) {
      // getQuotes returns array of quote objects
      const arr = Array.isArray(r.data) ? r.data : (r.data.data || []);
      arr.forEach(q => {
        const sym = q.symbol || q.Symbol;
        const price = parseFloat(q.lastPrice || q.LastPrice || q.last || 0);
        if (sym && price > 0) snap.prices[sym] = price;
      });
    } else {
      // Fallback to individual calls for this batch
      for (const sym of batch) {
        const r2 = await fetchNSE('/api/quote-equity?symbol='+encodeURIComponent(sym));
        if (r2.ok && r2.data?.priceInfo?.lastPrice) {
          snap.prices[sym] = r2.data.priceInfo.lastPrice;
        }
        await new Promise(r3 => setTimeout(r3, 150));
      }
    }
    await new Promise(r2 => setTimeout(r2, 300)); // 300ms between batches
  }
  ...
  if (yahooR.ok && yahooR.data) {
    try {
      const results = yahooR.data?.quoteResponse?.result || [];
      results.forEach(q => {
        const p = q.regularMarketPrice;
        if (!p) return;
        if (q.symbol==='USDINR=X') snap.usdInr = parseFloat(p);
        if (q.symbol==='BZ=F')     snap.brent  = parseFloat(p);
        if (q.symbol==='GC=F')     snap.gold   = parseFloat(p);
        if (q.symbol==='SI=F')     snap.silver = parseFloat(p);
      });
      if (snap.usdInr || snap.brent) {
        snap.fetchSuccess.push(`yahoo:USD/INR=${snap.usdInr},Brent=${snap.brent}`);
        console.log(`Yahoo: USD/INR=${snap.usdInr}, Brent=$${snap.brent}`);
      } else {
        snap.fetchErrors.push('yahoo:no_prices_in_response raw:'+JSON.stringify(results).slice(0,200));
      }
    } catch(e) {
      snap.fetchErrors.push('yahoo:parse_error:'+e.message);
    }
  } else {
    snap.fetchErrors.push('yahoo:'+(yahooR.error||'no_data'));
  }
